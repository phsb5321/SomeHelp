#include <bits/stdc++.h>

using namespace std;

class Node
{
public:
    int key;
    Node *left;
    Node *right;
    int height;
    int numFilhos;
};

int height(Node *N)
{
    if (N == nullptr)
        return 0;
    return N->height;
}

int max(int a, int b)
{
    return (a > b) ? a : b;
}

Node *newNode(int key)
{
    Node *node = new Node();
    node->key = key;
    node->left = nullptr;
    node->right = nullptr;
    node->height = 1;
    node->numFilhos = 0;
    return (node);
}

Node *rightRotate(Node *y)
{
    Node *x = y->left;
    Node *T2 = x->right;
    x->right = y;
    y->left = T2;
    y->height = max(height(y->left),
                    height(y->right)) +
                1;
    x->height = max(height(x->left),
                    height(x->right)) +
                1;

    x->numFilhos = x->left->numFilhos +
                   x->right->numFilhos;

    y->numFilhos = y->left->numFilhos +
                   y->right->numFilhos;
    return x;
}

Node *leftRotate(Node *x)
{
    Node *y = x->right;
    Node *T2 = y->left;
    y->left = x;
    x->right = T2;
    x->height = max(height(x->left),
                    height(x->right)) +
                1;
    y->height = max(height(y->left),
                    height(y->right)) +
                1;
    x->numFilhos = x->left->numFilhos +
                   x->right->numFilhos;

    y->numFilhos = y->left->numFilhos +
                   y->right->numFilhos;
    return y;
}

int getBalance(Node *N)
{
    if (N == nullptr)
        return 0;
    return height(N->left) - height(N->right);
}

// Recursive function to insert a key
// in the subtree rooted with node and
// returns the new root of the subtree.
Node *insert(Node *node, int key)
{
    /* 1. Perform the normal BST insertion */
    if (node == nullptr)
        return (newNode(key));

    if (key < node->key)
        node->left = insert(node->left, key);
    else if (key > node->key)
        node->right = insert(node->right, key);
    else // Equal keys are not allowed in BST
        return node;

    /* 2. Update height of this ancestor node */
    node->height = 1 + max(height(node->left),
                           height(node->right));

    node->numFilhos = 1 + node->right->numFilhos +
                      node->left->numFilhos;

    /* 3. Get the balance factor of this ancestor
        node to check whether this node became
        unbalanced */
    int balance = getBalance(node);

    // If this node becomes unbalanced, then
    // there are 4 cases

    // Left Left Case
    if (balance > 1 && key < node->left->key)
        return rightRotate(node);

    // Right Right Case
    if (balance < -1 && key > node->right->key)
        return leftRotate(node);

    // Left Right Case
    if (balance > 1 && key > node->left->key)
    {
        node->left = leftRotate(node->left);
        return rightRotate(node);
    }

    // Right Left Case
    if (balance < -1 && key < node->right->key)
    {
        node->right = rightRotate(node->right);
        return leftRotate(node);
    }

    /* return the (unchanged) node pointer */
    return node;
}

void preOrder(Node *root)
{
    if (root != nullptr)
    {
        cout << root->key << " ";
        preOrder(root->right);
        preOrder(root->left);
    }
}

void inOrder(Node *root)
{
    string str;
    if (root == nullptr)
    {
        return;
    }
    inOrder(root->left);
    cout << to_string(root->key) + " ";
    inOrder(root->right);
}

int getChildren(Node *root, int num = 1)
{
    return root->height;
    // if (root == nullptr)
    // {
    //     return 0;
    // }
    // int index = 0;
    // if (root->left != nullptr)
    //     index += getChildren(root->left, num);
    // if (root->right != nullptr)
    //     index += getChildren(root->right, num);
    // num += index;
    // return num;
}

int search(Node *root, int value, int index = 0)
{
    if (root->key == value && root->left == nullptr)
    {
        return index;
    }
    else if (root->key == value)
    {
        index += getChildren(root->left);
        return index;
    }
    else if (root->left == nullptr && root->right == nullptr)
        return pow(-10, 5);

    if (value < root->key)
    {
        index = search(root->left, value, index);
    }
    else
    {
        if (root->left != nullptr)
            index += getChildren(root->left);
        index += search(root->right, value, 1);
    }
    return index;
}

int main()
{
    Node *root = nullptr;
    string str_builder;

    int num_of_cases;
    scanf("%d\n", num_of_cases);

    int comand;
    int input;
    for (int a = 0; a < num_of_cases; a++)
    {
        scanf("%d %d\n", comand, input);
        switch (comand)
        {
        case 1:
            root = insert(root, input);
            break;
        case 2:
            int index = search(root, input);
            if (index < 0)
                str_builder += "Data tidak ada\n";
            else
                str_builder = to_string(index) + "\n";
            break;
        }
    }

    cout << str_builder;
    return 0;
}